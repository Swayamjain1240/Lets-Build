...
export const removeTeamMember = async (projectId, ownerId, memberUserId) => {
  const project = await Project.findById(projectId);
  ...
  if (project.owner.toString() !== ownerId.toString()) {
    const error = new Error('Only project owners can remove team members');
    error.statusCode = 403;
    throw error;
  }

  if (ownerId.toString() === memberUserId.toString()) {
    const error = new Error('Project owner cannot remove themselves');
